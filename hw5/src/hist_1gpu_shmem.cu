...
__global__ void hist_shmem(float *data, const long N, unsigned int *hist,
                           const int bins, const float Rmin,
                           const float binsize) {

    // use shared memory and atomic addition

    extern __shared__ unsigned int
        temp[]; // assume the blocksize is equal to the total # bins
    temp[threadIdx.x] = 0;
    __syncthreads();
...
int main(void) {
    ...
    if (bins > 1024) {
        printf(
            "The number of bins is set to # of threads per block < 1024 ! \n");
        exit(0);
    }
    ...
    if (threadsPerBlock != bins) {
        printf("The number of threads per block must be equal to the number of "
               "bins ! \n");
        exit(0);
    }
    fflush(stdout);
    ...
    hist_shmem<<<blocksPerGrid, threadsPerBlock, sm>>>(data_d, N, hist_d, bins,
                                                       Rmin, binsize);
...
void RandomExponential(float *data,
                       long n) // RNG with uniform distribution in (0,1)
{
    for (long i = 0; i < n; i++) {
