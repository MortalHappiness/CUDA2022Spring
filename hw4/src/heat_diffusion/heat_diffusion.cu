...
__global__ void heat_diffusion(float *phi0_old, float *phiL_old,
                               float *phiR_old, float *phiB_old,
    ...
    if (x == 0) { // left boundary of the sub-lattice
        if (phiL_old !=
            NULL) { // if it is not the left boundary of the entire lattice
        ...
        if (phiR_old !=
            NULL) { // if it is not the right boundary of the entire lattice
            l = phi0_old[site - 1];
            r = phiR_old[y * Lx];
        } else
            skip = 1;
    ...
    if (y == 0) { // bottom boundary of the sub-lattice
        if (phiB_old !=
            NULL) { // if it is not the bottom boundary of the entire lattice
        ...
        if (phiT_old !=
            NULL) { // if it is not the top boundary of the entire lattice
            b = phi0_old[site - Lx];
            t = phiT_old[x];
        } else
            skip = 1;
...
#pragma omp parallel private(cpu_thread_id)
    {
        ...
        int cpuid_l =
            ((cpuid_x + NGx - 1) % NGx) + cpuid_y * NGx; // GPU on the left
        ...
        int cpuid_b =
            cpuid_x + ((cpuid_y + NGy - 1) % NGy) * NGx; // GPU on the bottom
...
#pragma omp parallel private(cpu_thread_id)
        {
            ...
            dR_old = (cpuid_x == NGx - 1) ? NULL
                                          : d_old[cpuid_x + 1 + cpuid_y * NGx];
            dB_old =
                (cpuid_y == 0) ? NULL : d_old[cpuid_x + (cpuid_y - 1) * NGx];
            dT_old = (cpuid_y == NGy - 1)
                         ? NULL
                         : d_old[cpuid_x + (cpuid_y + 1) * NGx];

            heat_diffusion<<<blocks, threads, sm>>>(d0_old, dL_old, dR_old,
                                                    dB_old, dT_old, d0_new,
                                                    d_C[cpu_thread_id]);
            cudaDeviceSynchronize();

            cudaMemcpy(h_C + bx * by / NGPU * cpu_thread_id, d_C[cpu_thread_id],
