...
__global__ void poisson(float *phi_old, float *phi_new, float *rho, float *C,
    ...
    if ((x == 0) || (x == Nx - 1) || (y == 0) || (y == Ny - 1) || (z == 0) ||
        ...
        if (flag) {
            ...
            top = phi_old[zp1];
            phi_new[site] =
                (top + bottom + left + right + front + back + rho[site]) / 6;
        } else {
            left = phi_new[xm1];
            right = phi_new[xp1];
            back = phi_new[ym1];
...
int main(void) {
    ...
    // The total number of threads in the grid is equal to the total number of
    // lattice sites
    ...
    if ((CPU == 1) || (CPU == 2)) {
        ...
        int sm = tx * ty * tz *
                 sizeof(float); // size of the shared memory in each block
                    ...
                    for (int y = 0; y < L; y++) {
                        for (int z = 0; z < L; z++) {
                            if (x == 0 || x == L - 1 || y == 0 || y == L - 1 ||
                                ...
                                bottom = h_old[zm1];
                                top = h_old[zp1];
                                h_new[site] = (top + bottom + left + right +
                                               front + back + h_rho[site]) /
                                              6;
                    ...
                    for (int y = 0; y < L; y++) {
                        for (int z = 0; z < L; z++) {
                            if (x == 0 || x == L - 1 || y == 0 || y == L - 1 ||
                                ...
                                bottom = h_new[zm1];
                                top = h_new[zp1];
                                h_old[site] = (top + bottom + left + right +
                                               front + back + h_rho[site]) /
                                              6;
